# 构建有层次的索引：提升大规模数据检索效率

> 本文是 [refine-rag](https://github.com/zonezoen/refine-rag) 系列教程的第十一篇，我们来学习索引优化中的层次化索引技术。
...
前面我们学习了"从小块到大上下文"技术，解决了检索精度与上下文完整性的矛盾。但当数据规模增大到数万甚至数十万条时，即使是优化过的检索系统也会变得缓慢。

这就像在一个巨大的超市里找东西，如果没有分区标识，你需要在所有货架上逐一查找。层次化索引就是为数据建立"分区导航"，先快速定位到相关区域，再在小范围内精确查找，大幅提升检索效率。
...
层次化索引通过"先粗略定位，再精确查找"的策略，显著提升大规模数据的检索效率。双层索引适合多表/多文档场景，递归检索适合结构化数据，粗中有细适合超大规模数据集。合理使用层次化索引，可以将检索速度提升5-10倍，同时保持高精度。
